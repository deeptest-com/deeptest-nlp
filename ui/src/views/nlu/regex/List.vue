...
export default {
  ...
  methods: {
    ...
    create () {
      ...
      this.$router.push('/nlu/regex/0/edit')
    },
    edit (record) {
      this.visible = true
      this.mdl = { ...record }

      this.$router.push('/nlu/regex/' + record.id + '/edit')
    },
    detail (record) {
      this.visible = true
      this.mdl = { ...record }

      this.$router.push('/nlu/regex/' + record.id + '/items')
    },
    disable (record) {
      disableRegex(record).then(json => {
        console.log('disableRegex', json)
        this.$refs.table.refresh(false)
      })
    },
    confirmRemove (record) {
      removeRegex(record).then(json => {
        console.log('removeRegex', json)
        this.$refs.table.refresh(false)
      })
    },
    cancelRemove (e) {
