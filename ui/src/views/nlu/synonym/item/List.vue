...
export default {
  ...
  props: {
    synonymId: {
      type: Number,
      default: function () {
        return parseInt(this.$route.params.synonymId)
      }
    }
  },
  columns: [],
  ...
  data () {
    return {
      ...
      models: [],
      loadData: parameter => {
        const requestParameters = Object.assign({ synonymId: this.synonymId }, parameter, this.queryParam)
  ...
  methods: {
    ...
    resort (evt) {
      ...
      resortSynonymItem(src.id, target.id, this.synonymId).then(json => {
